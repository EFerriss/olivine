...
#%% SC1-2 hydrated
wb = SC.wb_800C_hyd
spec2 = wb.average_spectra()
spec2.fname = 'SC1-2-average'
baseline = {'abs_smear_high':10, 'wn_low':3200, 
             'curvature':0.05}
spec2.make_baseline(**baseline)
spec2.save_baseline(folder=SC.FTIR_file_location)
wb.make_baselines(**baseline)
wb.save_baselines()
#wb.make_baselines(abs_smear_low=10, abs_smear_high=10, wn_low=3425)
#wb.plot_showbaselines()
wb.save_baselines()