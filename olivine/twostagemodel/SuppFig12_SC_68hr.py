...
water = 14.
peaks = SC.peaks
initial = SC.wb_800C_43hr
final = SC.wb_800C_68hr
wb_list = [initial, final]

# change the time
final.time_seconds = final.time_seconds - initial.time_seconds

for wb in wb_list:
    # updated the initial
    for idx in range(3):
        wb.profiles[idx].initial_profile = initial.profiles[idx]
    wb.get_baselines()
    wb.make_areas()
...
fig.savefig(SC.thisfolder+'..\\twostagemodel\SuppFig12_SC_68hr.jpg', 
            dpi=400, format='jpg')   
